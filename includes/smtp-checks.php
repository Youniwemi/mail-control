...
function test_spf_record( string $domain, string $smtp_host, array $report ) {
	...
	if ( $spf_count == 0 ) {
		...
		if ( strpos( $spf_field, $smtp_host ) > 0 ) {
			...
			$report[] = sprintf( __( 'Good! Your SMTP host %1$s seems to be allowed to send emails for your domain %2$s', 'mail-control' ), $smtp_host, $domain );
			$report[] = '</p>';
			$spf_ok   = true;
		} else {
			$report[] = '<p class="notice notice-error">' . $record . '<br/>';
			$report[] = sprintf( __( 'Aie Aie! Your SMTP host %1$s doesn\'t seem to be allowed to send emails for your domain %2$s', 'mail-control' ), $smtp_host, $domain );
	...
	$report[]   = '<p class="notice notice-info"><strong>' . sprintf( __( 'Please keep in mind that this feature is experimental, you can find more information on SPF here %1$s, and use this wizard here %2$s to compose your record', 'mail-control' ), $spf_info, $spf_wizard ) . '</strong></p>';
	return array( $spf_ok, $report );
}
