...
add_filter( 'mail_control_settings', function ( $settings ) {
    ...
    if ( is_woocommerce_active() ) {
        $settings['EMAIL_CUSTOMIZER']['fields'][] = [
            'id'          => 'WOOCOMMERCE_ENABLED',
            'type'        => 'checkbox',
            'title'       => __( 'Customize Woocommerce emails', 'mail-control' ),
            'description' => __( 'If you don\'t check this option, woocommerce emails won\'t be customized, check this to allow mail-control to customize woocommerce emails.', 'mail-control' ),
        ];
    }
    return $settings;
} );
...
function get_my_settings( string $key = null )
{
    static  $settings ;
    if ( $settings == null ) {
        $settings = array_merge( get_defaults(), get_option( 'mc_customizer', [] ) );
    }
...
function get_font_family( $key = null )
{
    ...
    if ( $font_families == null ) {
        $font_families = apply_filters( 'mc_customizer_font_families', [
...
function get_customizer_settings()
{
    ...
    if ( $settings == null ) {
        $sections = apply_filters( 'mc_customizer_sections', [
        ...
        $settings = apply_filters( 'mc_customizer_settings', [
            [
                'id'      => 'email_type',
...
function customize_preview_init( $wp_customize )
{
    ...
    remove_hooks_except( 'customize_preview_init', [
...
{
    [ $settings, $sections ] = get_customizer_settings();
    // Add settings first
    foreach ( $settings as $config ) {
        ...
        $wp_customize->add_setting( $setting_id, [
            ...
            'sanitize_callback' => ( isset( $sanitize_callback ) ? [ get_control_class( $control ), $sanitize_callback ] : '' ),
        ] );
    }
    ...
    $wp_customize->add_panel( 'mc_customizer-panel', [
        'title'      => __( 'Email Customizer', 'mail-control' ),
        'capability' => MC_PERMISSION_MANAGER,
    ] );
    $count = 0;
    ...
    foreach ( $settings as $config ) {
        ...
        $wp_customize->add_control( new $control( $wp_customize, $setting_id, [
            'settings'        => $setting_id,
            'label'           => $label,
            'type'            => $type,
        ...
        if ( $wp_customize->selective_refresh && $selectors ) {
            $wp_customize->selective_refresh->add_partial( "mc_customizer[{$id}]", [
                'selector'        => $selectors,
...
add_action( 'settings_ready_mc', function () {
    ...
    if ( MC_CUSTOMIZING_EMAIL || MC_PREVIEWING_EMAIL ) {
        ...
        add_action( 'mc_header', function () {
            ...
            wp_enqueue_global_styles();
            wp_print_styles( [ 'wp-block-library', 'wp-block-library-theme', 'global-styles' ] );
            // remove our filter
