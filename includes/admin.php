...
function show_email_table() {
    $is_manager = current_user_can( MC_PERMISSION_MANAGER );
    if ( $is_manager ) {
        if ( isset( $_GET['cleanup_logs_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['cleanup_logs_nonce'] ) ), 'cleanup_logs' ) ) {
            if ( isset( $_GET['empty_logs'] ) && $_GET['empty_logs'] === '1' ) {
                do_action( 'mail_control_cleanup_logs' );
            }
        }
        $url = admin_url( 'admin.php?page=mail-control&empty_logs=1' );
        $clean_url = wp_nonce_url( $url, 'cleanup_logs', 'cleanup_logs_nonce' );
    }
    ...
    if ( $is_manager ) {
        ?>
			<a onclick="return confirm('Are you sure you want to clean up email logs according to your retention settings?');" href="<?php 
        echo esc_url( $clean_url );
        ?>" class="page-title-action"><?php 
        echo esc_html__( 'Cleanup Emails', 'mail-control' );
        ?> </a>
		<?php 
    }
    ?>
	...
	<style>
		...
		.wrap>h1 { display:inline-block; }
		td.column-content { max-width:300px; }
	</style>
	<script>
...
function onboarding_notice(  $is_welcome = false  ) {
    ...
    if ( 'index.php' !== $pagenow ) {
        $class_safe .= ' notice notice-info is-dismissible';
        $show_dismiss = true;
    }
    // Setup states.
